...
export class LocationInventoryDataTableComponent implements OnInit {
  ...
  loadNodes(event?: any, allOptionsClear = false) {
    ...
    // this.finalFilterdArr['advanceFilter'] = filterArr;
    ...
    if (!Array.isArray(this.finalFilterdArr['advanceFilter'])) {
      this.finalFilterdArr['advanceFilter'] = [...filterArr];
        ...
        filterArr.forEach(filter => existingFilters.set(JSON.stringify(filter), filter));
    ...
    const data = {
      ...
      maximumRows: 100,
      CompanyLocationId: this.contactDataEmit.Id,
      ...(this.contactDataEmit.Id ? { CompanyLocationId: this.contactDataEmit.Id } : {}),

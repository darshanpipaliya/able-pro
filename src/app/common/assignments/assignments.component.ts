...
export class AssignmentsComponent implements OnInit, OnDestroy {
  ...
  @Input() isAssignmentsTab: any;
  @Output() linkAssignmentData = new EventEmitter<any>();
  @Output() onLinkedAssignment = new EventEmitter<any>();
  ...
  ngOnDestroy(): void {
    ...
    this._unsubscribeLocations.complete();
    this._unsubscribePeoples.next(null);
    this._unsubscribePeoples.complete();
  }
          ...
          .subscribe(
            async (data: any) => {
              ...
              if (data && data.Data.$values.length > 0) {
                ...
                this.gridApiLocation.showNoRowsOverlay();
              }
            },
            (error) => {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (!isValuesUndefined(result)) {
        ...
        if (this.primaryLocationId) {
          this.onAgGridReadyLocation(this.gridApiLocation);
        }
      }
    });

...
export class CustomDualListComponent implements DoCheck, OnChanges {
  ...
  static AVAILABLE_LIST_NAME = 'available';
	...
	static DEFAULT_FORMAT = { add: 'Add', remove: 'Remove', all: 'All', none: 'None', direction: CustomDualListComponent.LTR, draggable: true };

	@Input() id = `dual-list-${this.nextId++}`;
	@Input() key = '_id';
	@Input() display = '_name';
	@Input() height = '100px';
	@Input() filter = false;
	@Input() format = CustomDualListComponent.DEFAULT_FORMAT;
	@Input() sort = false;
  ...
  @Input() presenter:any;
	...
	constructor(private differs:IterableDiffers) {
		this.available = new BasicList(CustomDualListComponent.AVAILABLE_LIST_NAME);
		this.confirmed = new BasicList(CustomDualListComponent.CONFIRMED_LIST_NAME);
	}
	...
	ngOnChanges(changeRecord: {[key:string]:SimpleChange}) {
		...
		if (changeRecord['sort']) {
			if (changeRecord['sort'].currentValue === true && this.compare === undefined) {
				...
				this.compare = undefined as unknown as compareFunction;
			}
		}
		...
		if (changeRecord['format']) {
			...
			if (typeof(this.format.direction) === 'undefined') {
				this.format.direction = CustomDualListComponent.LTR;
			}

			if (typeof(this.format.add) === 'undefined') {
				this.format.add = CustomDualListComponent.DEFAULT_FORMAT.add;
			}

			if (typeof(this.format.remove) === 'undefined') {
				this.format.remove = CustomDualListComponent.DEFAULT_FORMAT.remove;
			}

			if (typeof(this.format.all) === 'undefined') {
				this.format.all = CustomDualListComponent.DEFAULT_FORMAT.all;
			}

			if (typeof(this.format.none) === 'undefined') {
				this.format.none = CustomDualListComponent.DEFAULT_FORMAT.none;
			}

			if (typeof(this.format.draggable) === 'undefined') {
				this.format.draggable = CustomDualListComponent.DEFAULT_FORMAT.draggable;
			}
		}
	...
	updatedSource() {
		...
		if (this.source !== undefined) {
			this.sourceDiffer = this.differs.find(this.source).create(undefined);
		}
	}

	updatedDestination() {
		if (this.destination !== undefined) {
			this.destinationDiffer = this.differs.find(this.destination).create(undefined);
		}
	}

	direction() {
		return this.format.direction === CustomDualListComponent.LTR;
	}

...
import { AttributeInputRender } from '../add-wireline/review-service-popup/attribute-input.component';
...
export class AddServiceTypeComponent implements OnInit {
  ...
  attributesArray:any = [];
  constructor(private locationService: LocationService, private dialogRef: MatDialogRef<AddServiceTypeComponent>, private wirelineService: WirelineService, private _formBuilder: FormBuilder,
    ...
    if (!matchRecord) {
      ...
      // else {
        if (this.data.ServiceTypeId && !this.getInventoryFormData) {
          ...
          this.wirelineService.inventoryattributesAssign(this.attributeData).pipe(takeUntil(this._unsubscribeInventory)).subscribe((data) => {
            if (data.Success) {

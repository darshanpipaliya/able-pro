...
@Component({
  ...
  standalone: true,
  providers: [DashboardService, LocationService],
  imports: [
    CommonModule,
    SharedModule,
...
export class FinanceDashboardComponent implements OnInit {
  ...
  getinvoiceSumByVendor(event: { value: any; }, pageSize = 10, pageNumber = 1, searchText = '') {
    ...
    this.dashboardService.invoiceSumByVendor(data).pipe(takeUntil(this._unsubscribeSpend)).subscribe(async (res: any) => {
      ...
      this.rowData = this.rowData?.forEach((element: { [x: string]: string | any; differenceP: number; differenceD: any; }, index: any) => {
  ...
  loadChartInTable(totalRecords: any) {
    this.rowData?.forEach(async (row: { trend: any; }, index: string) => {
  ...
  async loadChartSpendSummarybyServiceType(series: string | any[], category: any): Promise<void> {
    if (this.isBrowser) {
      try {

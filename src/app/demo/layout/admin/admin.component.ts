...
export class AdminComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    if (this.userInfo?.headerLogo) {
      const imageData = this.userInfo?.headerLogo;
      let objectURL = imageData;
      if (!objectURL.startsWith('data:')) {
        objectURL = 'data:png;base64,' + objectURL
      }
      this.headerImageSrc = this.sanitizer.bypassSecurityTrustUrl(objectURL);
    } else {
      this.headerImageSrc = null;
    }
    ...
    if (drawerContent) {
      if (layout === VERTICAL) {
        if (this.windowWidth > 1025) {
          drawerContent.style.marginLeft = this.rtlMode === true ? '0px' : '230px';
          drawerContent.style.marginRight = this.rtlMode === true ? '230px' : '0px';
        }

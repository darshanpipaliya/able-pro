...
import { extractData, filterOptionsDate, filterOptionsNumber, filterOptionsText, onChangeEndDate } from 'src/app/services/common-p-table';
...
@Component({
  selector: 'app-inventory-service-number',
  templateUrl: './inventory-service-number.component.html',
  styleUrls: ['./inventory-service-number.component.scss'],
  imports: [SharedModule, PrimgModule]
})
export class InventoryServiceNumberComponent implements OnInit {

  @Input() gridRowData: any;
  @Input() rowDataInventory: any;
  ...
  @Output() gridRecords: EventEmitter<any> = new EventEmitter();
  ...
  setCols() {
    ...
    this.cols = [
      createColumn(1, '50px', true, 'checkbox', '', 'checkbox', ''),      
      createColumn(2, '150px', true, 'text', 'Inventory', 'ServiceNumber', 'Billing ID'),
      createColumn(3, '150px', true, 'text', 'Account', 'MainAccountNumber', 'Main Billing Account Number'),
      createColumn(3, '150px', false, 'text', '', 'SubAccountNumber', 'Sub Account Number', 'open'),
      createColumn(3, '150px', false, 'text', '', 'PayableAccountNumber', 'Payable Account Number', 'open'),
      createColumn(4, '150px', true, 'numberFilter', 'Charge', 'TotalCurrentChargesDisplay', 'Charge'),
      createColumn(5, '150px', true, 'text', 'Vendor Product', 'VendorProductTypeName', 'Vendor Product'),
      createColumn(5, '150px', false, 'text', '', 'Service', 'Service', 'open'),
      createColumn(5, '150px', false, 'text', '', 'ServiceType', 'Service Type', 'open'),
      createColumn(5, '150px', false, 'text', '', 'Product', 'Product', 'open'),
      createColumn(5, '150px', false, 'text', '', 'ProductType', 'Product Type', 'open'),
      createColumn(6, '150px', true, 'text', 'Status', 'InventoryStatusDisplayText', 'Status'),
    ];
  ...
  loadNodes(event?: any, allOptionsClear = false, fromHeaderCheckBox = false) {
    ...
    const data = {
      ...
      ...(this.gridRowData.InvoiceId) ? { invoiceId: this.gridRowData.InvoiceId } : {},
      ...(this.gridRowData.VendorAccountId) ? { VendorAccountId: this.gridRowData.VendorAccountId } : {},
      ...(this.gridRowData.CustomerAccountId) ? { customerAccountId: this.gridRowData.CustomerAccountId } : {},
      ...{IsTotalNeed: true},
      ...{ withChild: false },
      ...{ isForDistrubation: true },
      ...{ isFromChargeDistributionStep6: true },
      // ...{ getAll: true }
    };

    if (fromHeaderCheckBox) {
      data.getAll = true;
    }
  ...
  handleResponse(response: any, allOptionsClear: any) {
    ...
    if (response?.Data?.$values?.length) {
      ...
      if(this.selectedNodes.length > 0) {
        this.selectedRecords = this.getCheckedNodesArr(this.files, this.selectedNodes);
      }
  ...
  getCheckedNodesArr(nodes: any[], selectedNodes: any[]): any {
    return nodes.filter(node => selectedNodes.some(selectedNode => selectedNode.VendorProductInventoryId === node.VendorProductInventoryId));
  }
  handleError() {
    this.loading = false;
    this.files = [];
  ...
  selectChildCheckbox() {
    ...
    this.selectedRowsEmit.emit(this.selectedRecords);
  }

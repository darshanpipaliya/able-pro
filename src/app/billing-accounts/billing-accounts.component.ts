...
export class BillingAccountsComponent implements OnInit, AfterViewInit {
  ...
  onAgGridReady() {
    ...
    let data = {}
    this.locationService.getBillingAccount(data).pipe(takeUntil(this._unsubscribeGRid)).subscribe((data) => {
  ...
  loadNodes(allOptionsClear = false) {
    ...
    this.locationService.billingaccountsLogged(data)
  ...
  onNodeExpand(event: any) {
    ...
    if (!node.children || node.children.length === 0) {
      ...
      const data = {
        ...
        maximumRows: 10000,
        BillingAccountHierarchyId: node.data.BillingAccountHierarchyId,
        ...(this.sorting ? { OrderBy: this.sorting, SortOrder: this.sortingType } : {}),
      };

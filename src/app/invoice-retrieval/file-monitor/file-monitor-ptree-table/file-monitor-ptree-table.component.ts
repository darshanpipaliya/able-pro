import { Component, EventEmitter, Input, OnInit, Output, NgModule, ViewChild, ElementRef, AfterViewInit, HostListener } from '@angular/core';
...
@Component({
  selector: 'app-file-monitor-ptree-table',
  templateUrl: './file-monitor-ptree-table.component.html',
  styleUrls: ['./file-monitor-ptree-table.component.scss'],
  imports: [SharedModule, PrimgModule]
})


export class FileMonitorPtreeTableComponent implements OnInit, AfterViewInit {
  @Output() rowCellDoubleClicked: EventEmitter<any> = new EventEmitter();
  @Output() exportAccountData: EventEmitter<any> = new EventEmitter();
  @Output() disableExcelbtn: EventEmitter<any> = new EventEmitter();
  ...
  setCols() {
    ...
    this.cols = [
      createColumn(1, '150px', true, 'numberFilter', 'Import ID', 'ImportID', 'Import ID','close' ,1, 150, true, true, false),
  
      createColumn(2, '230px', true, 'text', 'Status', 'ImportStatus', 'Sandbox Status','close' ,1, 150, true, true, false),
      createColumn(2, '210px', false, 'text', '', 'ImporttoSandboxStatusValue', 'Presandbox Status','close' ,1, 150, true, true, false),

      createColumn(3, '190px', true, 'text', 'File', 'ImportTypeDisplayName', 'File Source','close' ,1, 150, true, true, false),
      createColumn(3, '175px', false, 'text', '', 'ImportTypeDescription', 'File Description','close' ,1, 150, true, true, false),
      createColumn(3, '165px', false, 'text', '', 'ImportFileName', 'File Name','close' ,1, 150, true, true, false),
      createColumn(3, '155px', false, 'text', '', 'ImportGeneratedFileName', 'System File Name','close' ,1, 150, true, true, false),
      createColumn(3, '200px', false, 'numberFilter', '', 'ImportFileAttempCount', 'Attempts','close' ,1, 150, true, true, false),
      createColumn(3, '200px', false, 'dateFilter', '', 'ImportCreatedDate', 'File Date & Time','close' ,1, 150, true, true, false),

      createColumn(4, '180px', true, 'text', 'Account', 'VendorAccountName', 'Vendor', 'close',1, 150, true, true, false),
      createColumn(4, '170px', false, 'text', '', 'ExpectedCustomerName', 'Customer', 'close',1, 150, true, true, false),
    ];
  ...
  loadNodes(event: any, allOptionsClear = false) {
    ...
    const data = {
      ...
      maximumRows: this.pTableContain.first ==1 ? 100 : this.pTableContain.first + 99,
    ...
    this.locationService.getFileMonitoring(data)
  ...
  handleResponse(response: any, allOptionsClear: any) {
    ...
    this.files.length > 0 ? this.disableExcelbtn.emit(false ) : this.disableExcelbtn.emit(true);
  }
  ...
  extractDataAndLeaf = (item: any) => {
    return {
      ...
      leaf: item.SubFileGridData === null ? true : false,
    };
  }
  ...
  onNodeExpand(event: any) {
    ...
    if (!node.children || node.children.length === 0) {
      ...
      node.children = event.node.data.SubFileGridData.$values.map(this.extractDataAndLeaf.bind(this));

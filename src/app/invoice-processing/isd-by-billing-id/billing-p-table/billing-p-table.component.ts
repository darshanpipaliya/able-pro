...
export class BillingPTableComponent implements OnInit {
  ...
  @Output() addCorrectionCall: EventEmitter<any> = new EventEmitter();
  ...
  constructor(public locationService: LocationService, public wirelineService: WirelineService, public sanboxService: SandBoxService,
  ...
  setCols() {
    ...
    this.cols = [
      createColumn(1, '200px', true, 'text', '', 'BillingId', 'Billing Id'),

      createColumn(2, '150px', true, 'text', 'Account', 'MainAccountNumber', 'Main Account', 'close'),
      createColumn(2, '150px', false, 'text', '', 'Subaccount', 'Sub Account', 'open'),

      createColumn(3, '220px', true, 'numberFilter', 'Summary Comparison', 'DifferenceDisplay', 'Difference', 'close'),
      createColumn(3, '176px', false, 'numberFilter', '', 'ChargeSummeryDisplay', 'Charge Summary', 'open'),
      createColumn(3, '169px', false, 'numberFilter', '', 'TotalChargeDisplay', 'Detail Summary', 'open'),

      createColumn(4, '164px', true, 'numberFilter', 'Charge Detail', 'ChargeDisplay', 'Itemized Detail', 'close'),

      createColumn(5, '189px', true, 'text', 'Charge Code', 'ChargeCodeName', 'Charge Code Name', 'close'),
      createColumn(5, '200px', false, 'text', '', 'ChargeCode', 'Charge Code', 'open'),
      createColumn(5, '182px', false, 'text', '', 'ChargeCodeCodeType', 'Charge Code Type', 'open'),
      createColumn(5, '180px', false, 'text', '', 'ChargeTypeName', 'Charge Type', 'open'),
      createColumn(5, '104px', false, 'text', '', 'quantity', 'Quantity', 'open'),
      createColumn(5, '175px', false, 'text', '', 'UnitOfMeasure', 'Unit of Measure', 'open'),
      createColumn(5, '175px', false, 'text', '', 'ChargeAdjustmentNote', 'Adjustment Note', 'open'),

      createColumn(6, '170px', true, 'text', 'Charge Location', 'ChargeLocationTypeName', 'Charge Location', 'close'),
      createColumn(7, '170px', true, 'text', 'Distribution', 'DistributionEventId', 'Distribution Event ID', 'close'),
    ];
    ...
    this.filesColumns = _.uniqBy(this.filesColumns, 'parentid');
  ...
  AddCorrection() {
    let data = this.selectedNodes.filter((item: any) => item.data?.HasParent === true).map((item: any) => item.data);
    this.addCorrectionCall.emit(data)
  }
  ...
  loadNodes(event?: any, allOptionsClear = false) {
    ...
    const data = {
      ...
      maximumRows: 100,
      // ...(this.sandBoxGridRowData.Id ? { CompanyLocationId: this.sandBoxGridRowData.Id } : {}),
    ...
    this.sanboxService
    .chargeValidationByBillingIds(this.sandBoxGridRowData.SBInvoiceId, data)
  ...
  onNodeExpand(event: any) {
    ...
    if (!node.children || node.children.length === 0) {
      ...
      const data = {
        ...
        maximumRows: 10000,
        billingAccountHiearchyId: node.data.BillingAccountHiearchyId,
        InventoryId: node.data.InventoryId
  ...
  setColumnDefs() {
    ...
    this.exportAccounts = {
      ...
      ExportToExcel: true,
      ...this.finalFilterdArr,
      // ...(this.rowData.PeopleId ? { PeopleId: this.rowData.PeopleId } : {}),
      ...(this.sorting ? { OrderBy: this.sorting, SortOrder: this.sortingType } : {})
    };

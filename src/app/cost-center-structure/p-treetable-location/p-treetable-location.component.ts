...
@Component({
  selector: 'app-p-treetable-location',
  templateUrl: './p-treetable-location.component.html',
  styleUrls: ['./p-treetable-location.component.scss'],
  imports: [SharedModule, PrimgModule]
})
export class PTreetableLocationComponent implements OnInit {
  ...
  onHeaderCheckboxChange(e: any) {
    ...
    if (this.files.length > 0 && this.headerCheckboxData) {
      ...
      this.selectedRecords.forEach((e: any) => {
        selectedInventory.push(e['data']['Id'])
      });
  ...
  selectChildCheckbox() {
    this.selectedRecords = [...new Map(this.selectedRecords.map(item => [item.data.LocationId, item])).values()];
    let a = this.selectedRecords.map(item => item.data);
    this.selectedRowsEmit.emit(a);
  ...
  setCols() {
    ...
    this.cols = [
      ...
      createColumn(2, '200px', true, 'text', 'Location Name', 'LocationName', 'Location Name'),
      createColumn(2, '200px', false, 'numberFilter', '', 'LocationId', 'Location Id'),

      createColumn(3, '200px', true, 'text', 'Location Type', 'LocationTypeDisplayName', 'Location Type'),

      createColumn(4, '200px', true, 'text', 'Address', 'Address1', 'Address One'),
      createColumn(4, '170px', false, 'text', '', 'Address2', 'Address Two'),
      createColumn(4, '100px', false, 'text', '', 'City', 'City'),
      createColumn(4, '158px', false, 'text', '', 'StateName', 'State/Province'),
      createColumn(4, '168px', false, 'text', '', 'PostalCode', 'Zip/Postal Code'),

      createColumn(5, '190px', true, 'text', 'Location Status', 'DisplayText', 'Location Status'),
  ...
  loadNodes(event?: any, allOptionsClear = false, fromHeaderCheckBox = false) {
    ...
    let data = {
      ...
      ...(this.primaryLocationId ? { LocationId: this.primaryLocationId } : {}),
    };

    if(this.action == 'add' && this.LocationId && this.fromPage === 'Invoice') {
      data['LocationId'] = this.LocationId;
      data['ForPreSelect'] = true;
    }

    if (this.fromPage === 'Invoice' && this.action == 'add') {
      delete data['costCenterStructureId']
    }
    if(this.action == 'edit' && this.LocationId) {
      data['LocationId'] = this.LocationId;
    }
    ...
    this.locationService.getCompanylocationsURL(data)
  ...
  getCheckedNodes(nodes: any): TreeNode[] {
    ...
    for (const node of nodes) {
      // if (this.existingLocation?.some(r => r === node.data['LocationId'])) {
      //   selected.push(node);
      // }

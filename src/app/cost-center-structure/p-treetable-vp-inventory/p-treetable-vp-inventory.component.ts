...
@Component({
  selector: 'app-p-treetable-vp-inventory',
  templateUrl: './p-treetable-vp-inventory.component.html',
  styleUrls: ['./p-treetable-vp-inventory.component.scss'],
  imports: [SharedModule, PrimgModule]
})
export class PTreetableVpInventoryComponent implements OnInit {

  @Input() selectedTem: any;
  @Input() serviceTypeIds: any;
  @Input() companyId: any;
  ...
  @Input() title: any;
  @Input() action: any;
  @Input() VendorProductInventoryId: any;
  @Input() VendorAccountId: any;
  @Input() primaryVPIId: any;
  ...
  onHeaderCheckboxChange(e: any) {
    ...
    if (this.files.length > 0 && this.headerCheckboxData) {
      ...
      this.selectedRecords.forEach((e: any) => {
        selectedInventory.push(e['data']['VendorProductInventoryId'])
      });
  ...
  selectChildCheckbox() {
    this.selectedRecords = [...new Map(this.selectedRecords.map(item => [item.data.VendorProductInventoryId, item])).values()];
    let a = this.selectedRecords.map(item => item.data);
    this.selectedRowsEmit.emit(a);
  ...
  setCols() {
    this.cols = [
      ...
      createColumn(5, '210px', true, 'text', 'Status', 'InventoryStatusDisplayText', 'Vendor Product Status'),

      createColumn(6, '183px', true, 'text', 'Current Assignment', 'LocationName', 'Location Name'),
      createColumn(6, '183px', false, 'numberFilter', '', 'LocationId', 'Location Id'),


      createColumn(7, '150px', true, 'text', 'Address', 'Address1', 'Address One'),
      createColumn(7, '150px', false, 'text', '', 'Address2', 'Address Two', 'open'),
      createColumn(7, '100px', false, 'text', '', 'City', 'City', 'open'),
      createColumn(7, '158px', false, 'text', '', 'StateName', 'State/Province', 'open'),
      createColumn(7, '168px', false, 'text', '', 'PostalCode', 'Zip/Postal Code', 'open'),

      createColumn(8, '200px', true, 'text', 'People', 'PeopleName', 'Name'),
      createColumn(8, '200px', false, 'text', '', 'PeopleEmail', 'Email', 'open')
    ];
  ...
  loadNodes(event?: any, allOptionsClear = false, fromHeaderCheckBox = false) {
    ...
    const data = {
      ...
      maximumRows: 100,
      ...(this.serviceTypeIds && this.serviceTypeIds.length > 0 ? { serviceTypeIds: this.serviceTypeIds } : {}),
      ...(this.productIds && this.productIds !== null ? { productIds: [this.productIds] } : {}),
      ...(this.companyId && this.companyId !== 'all' ? { companyId: this.companyId } : {}),
      ...(this.VendorAccountId && this.VendorAccountId !== '' ? { VendorAccountId: this.VendorAccountId } : {}),
      ...(this.CustomerAccountId && this.CustomerAccountId !== 'all' ? { CustomerAccountId: this.CustomerAccountId } : {}),
      ...(this.costCenterStructuresid  ? { costCenterStructuresid: this.costCenterStructuresid } : {}),
      ...(this.VendorProductInventoryId && this.action === 'add'  ? { VendorProductInventoryId: this.VendorProductInventoryId } : {}),
      ...(this.action === 'add'  ? { ForPreSelect: true } : {}),
      ...(this.headerCheckboxData ? { GetAll: this.headerCheckboxData } : {}),
      ...(this.sorting ? { OrderBy: this.sorting, SortOrder: this.sortingType } : {}),
      ...(this.primaryVPIId ? { VendorProductInventoryId: this.primaryVPIId } : {}),
    };


    if(this.action === 'edit' && this.VendorProductInventoryId) {
  ...
  handleResponse(response: any, allOptionsClear: any, fromHeaderCheckBox: any) {
    ...
    if (response?.Data?.$values?.length) {
      ...
      // node.data.InventoryLocationAtt == 'Yes' ? true : false
    ...
    this.cols.forEach((item: any) => {
      if (e.node.isparent && item.parent === e.node.parentid || !e.node.isparent && item.parent === e.node.parentid && item.childHeader === e.node.label) {
        ...
        if (e.node.isparent && this.cols.some((it: any) => it.parent === e.node.parentid && it.isChildren && it.isicon === 0)) {
          ...
          closedColumns = this.colsshow.some((it: any) => it.parent === e.node.parentid && it.columnGroupShow === 'close' && (!item.isChildren && it.childHeader === item.childHeader) || (item.isChildren && it.header === item.header));
        }

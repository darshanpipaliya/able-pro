...
export class AddNotesLocationComponent implements OnInit {
  ...
  setCols() {
    ...
    if (this.hasSsuperTemUsers) {
      this.cols = [
        ...
        createColumn(5, '140px', true, '', '', 'IsLocationNoteAttachment', 'Attachments'),
  ...
  ngOnInit(): void {
    ...
    if (this.hasSsuperTemUsers) {
      this.cols = [
        { field: 'LocationName', header: 'Location Name', width: '160px', spinner: true },
        { field: 'LocationNoteCreatedDate', header: 'Date', width: '104px', spinner: true },
        { field: 'Notes', header: 'Notes', width: '160px', spinner: true },
        { field: 'IsLocationNoteAttachment', header: 'Attachments', width: '140px', spinner: true },
        { field: 'LocationNoteStatus', header: 'Note Status', width: '132px', spinner: true },
        { field: 'NoteType', header: 'Note Type', width: '123px', spinner: true },
        { field: 'LocationNoteCreatedByWithEmail', header: 'Who', width: '200px', spinner: true },
      ];
    } else {
      this.cols = [
        { field: 'LocationName', header: 'Location Name', width: '160px', spinner: true },
  ...
  onHeaderCheckboxChange(e: any) {
    ...
    if (this.files.length > 0 && this.headerCheckboxData) {
      ...
      this.selectedRecords.forEach((e: any) => {
        selectedInventory.push(e['data']['LocationNoteId'])
      });
  ...
  setChangeLocationNotesStatus(type: any) {
    ...
    // const selectedLocationNotes: any = [];
    this.selectedRecords.forEach((e) => {
      this.locationId = this.locationaData.LocationId;
      selectedLocationNotesId.push(e['data']['LocationNoteId']);
      // selectedLocationNotes.push(e);
    });
    this.locationNoteIds = selectedLocationNotesId;
    // this.locationNotesData = selectedLocationNotes;
    ...
    if (this.selectedRecords.length == 0) {
      ...
      if (this.selectedRecords.length >= 1) {
        let isInActive = _.some(this.selectedRecords, (x: any) => x['data'].LocationNoteStatus == 'Inactive');
        let isActive = _.some(this.selectedRecords, (x: any) => x['data'].LocationNoteStatus == 'Active');
  ...
  loadNodes(event?: any, allOptionsClear = false) {
    if (!event?.sortField) {
      ...
      this.locationService.getLocationnotes(data).pipe(takeUntil(this._unsubscribeNotes)).subscribe(

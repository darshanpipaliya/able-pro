...
export class PTreeLocationComponent implements OnInit {
  ...
  setCols() {
    ...
    this.cols = [
      ...
      createColumn(currentParent, '50px', false, 'text', '', 'Alias', 'Location Alias', 'open'),
  ...
  handleNodeResponse(response: { _companyLocationDto: { $values: any[]; }; }, node: any) {
    this.innerLoading = false;

    if (response?._companyLocationDto?.$values?.length) {
      // Map response data to node children
      node.children = response._companyLocationDto.$values.map(extractDataAndLeaf.bind(this));

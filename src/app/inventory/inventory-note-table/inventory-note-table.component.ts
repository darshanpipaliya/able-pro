import { ChangeDetectionStrategy, Component, EventEmitter, HostListener, Input, OnDestroy, OnInit, Output, SimpleChanges, ViewChild } from '@angular/core';
import { MatDialog } from '@angular/material/dialog';
import { InventoryNotePopupComponent } from './inventory-note-popup/inventory-note-popup.component';
import { takeUntil } from 'rxjs/operators';
import { Subject } from 'rxjs';
import moment from 'moment';
import { AttachmentRenderer } from './attachment-renderer.component';
...
import { extractData, onChangeEndDate } from 'src/app/services/common-p-table';
...
@Component({
  selector: 'app-inventory-note-table',
  templateUrl: './inventory-note-table.component.html',
  styleUrls: ['./inventory-note-table.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [SharedModule, PrimgModule],
})
export class InventoryNoteTableComponent implements OnInit, OnDestroy {

  @Input() rowData: any;
  @Input() buttonAction: any;
  @Input() selectedButton: any;
  @Input() isInventoryNotes: any
  @Input() parentServiceID: any;
  ...
  private _unsubscribeDeactive: Subject<any> = new Subject<any>();
  headerCheckboxData = false;

  public inventoryRowData: any = [];
  inventoryCount: number;
  public isInventoryExist: boolean = false;
  ...
  constructor(private variableManageService: VariableManageService, public dialog: MatDialog, private wirelineService: WirelineService,
  ...
  setCols() {
    ...
    if (this.hasSsuperTemUsers) {
      this.cols = [
        ...
        createColumn(2, '200px', false, 'text', '', 'ServiceNumber', 'Service Number'),
        createColumn(3, '230px', false, 'text', '', 'VendorProductTypeName', 'Vendor Product'),
        createColumn(4, '160px', false, 'dateFilter', '', 'CreationDate', 'Date'),
        createColumn(5, '400px', false, 'text', '', 'NoteText', 'Note'),
        createColumn(6, '120px', false, 'text', '', 'IsAttachment', 'Attachment'),
        // createColumn(6, '190px', false, 'text', '', 'PrivateNoteDisplayText', 'Private'),
        createColumn(7, '130px', false, 'text', '', 'StatusDisplayText', 'Note Status'),
        createColumn(8, '130px', false, 'text', '', 'NoteType', 'Note Type'),
        createColumn(9, '300px', false, 'text', '', 'CreatedByUserWithEmail', 'Who'),
      ];
    } else {
      this.cols = [
        createColumn(1, '50px', true, '', '', 'checkbox', ''),
        ...
        createColumn(6, '120px', false, 'text', '', 'IsAttachment', 'Attachment'),
        createColumn(7, '130px', false, 'text', '', 'StatusDisplayText', 'Note Status'),
        createColumn(8, '300px', false, 'text', '', 'CreatedByUserWithEmail', 'Who')
      ];
    }
  ...
  onHeaderCheckboxChange(e: any) {
    ...
    if (this.files.length > 0 && this.headerCheckboxData ) {
      ...
      this.selectedRecords.forEach((e: any) => {
        this.inventoryId = e['data']['InventoryId'];
        selectedInventory.push(e['data']['InventoryNoteId'])
      });
  ...
  ngOnInit(): void {
    this.wirelinePageName.emit('InventoryNotes');
    this.isRoleAccess = rolePermission(['SuperTEMAdmin', 'SuperTEMManager', 'TEMAdmin', 'TEMManager']);
    this.hasSsuperTemUsers = this.locationService.isUserHasSuperTEMUsersRole();
  ...
  loadNodes(event?: any, allOptionsClear = false) {
    ...
    const data = {
      ...
      maximumRows: 100,
      IsTotalNeed: true,
      ...(this.rowData['CustomerAccountId'] ? { CustomerAccountId: this.rowData['CustomerAccountId'] } : {}),
      ...(this.rowData['InventoryId'] ? { InventoryId: this.rowData['InventoryId'] } : {}),
      ...(this.rowData['serviceIds'] ? { serviceIds: this.serviceIds } : {}),
    ...
    this.wirelineService.getInventoryNotes(data, this.rowData['InventoryId'])
  ...
  setColumnDefs() {
    ...
    this.exportAccounts = {
      ...
      ...this.finalFilterdArr,
      ...(this.sorting ? { OrderBy: this.sorting, SortOrder: this.sortingType } : {}),
      ...(this.serviceIds ? { serviceIds: this.serviceIds } : {}),
    };
      ...
      this.wirelineService.getChildinventoriesDetails(this.rowData['ChildInventoryId']).pipe(takeUntil(this._unsubscribeGetServiceDetail))
        .subscribe((data) => {
          if (data && data.Data) {
            ...
            this.wirelineService.getServiceDetail(data.Data['VendorProductInventoryId'])
              .pipe(takeUntil(this._unsubscribeGetServiceDetail))
              .subscribe((data) => {
                if (data && data.Data) {
                  this.parentServiceID = data.Data.ServiceNumber;
                }
              });
          } else {
          }
  ...
  addNotes() {
    const dialogRef = this.dialog.open(InventoryNotePopupComponent, {
      panelClass: ['width-900', 'popup-custom-design'],
      data: { rowData: this.rowData, parentServiceID: this.parentServiceID },
      disableClose: true
    });
  ...
  onBtnExportDataAsExcel() {
    const data: any = {};
    data['exportFilename'] = 'Assignments_People.xlsx';
    data['sheetName'] = 'Assignments_People';
    data['gridApi'] = this.gridApi;
    data['gridColumnApi'] = this.gridColumnApi;
  ...
  onSelectionChanged(event: any) {
    ...
    event.forEach((e: any) => {
      ...
      selectedInventory.push(e.InventoryNoteId)
    });
    this.selectedInventory = selectedInventory;
  }
  ...
  buttonClicked(rowData: any) {
    ...
    this.wirelineService.downloadInventoryNote(rowData.InventoryNoteId).subscribe({
      next: dataa => {
        let bolbUrl = URL.createObjectURL(dataa);
        var link = document.createElement("a");
        link.setAttribute("href", bolbUrl);

        link.setAttribute("download", rowData.GeneratedFileName);
  ...
  setInventoryNotes(type: any) {
    ...
    let isSameType = _.every(this.selectedRecords, (x: any) => x['data'].StatusDisplayText.toLowerCase() == type.toLowerCase())
    ...
    if (this.selectedRecords.length == 0) {
      ...
      let errorData: any = {
        ...
        icon: "fas fa-exclamation-circle",
        iconClass: "text-c-blue f-70",
        message: `Please select ${statusAlt} notes to change the status to ${this.selectedRecords[0]['data'].StatusDisplayText}`
      }
      this.dialog.open(ErrorWarningPopupComponent, { panelClass: 'error-warning', data: errorData });
      this.isSetAction.emit('');
    } else {
      if (this.selectedRecords.length >= 1) {
        let isInActive = _.some(this.selectedRecords, (x: any) => x['data'].StatusDisplayText == 'Inactive');
        let isActive = _.some(this.selectedRecords, (x: any) => x['data'].StatusDisplayText == 'Active');
        if (isInActive && type == 'inactive') {
          ...
          data['inventoryNoteIds'] = this.selectedInventory;
          data['active'] = (type == 'active') ? true : false;
          this._unsubscribeActive.next(null);
          this.wirelineService.inventorynotesStatus(data).pipe(takeUntil(this._unsubscribeActive)).subscribe((response) => {
            if (response.Success) {
              this.selectedRecords = [];
  ...
  public columnDefs = [
    ...
    {
      headerName: 'Private',
      field: 'PrivateNoteDisplayText',
      filter: 'agTextColumnFilter',
      editable: false,
      minWidth: 150,
      hide: this.isRoleAccess ? false : true,
      sortingField: 'PrivateNoteDisplayText'
    },
    {
      ...
      editable: false,
      minWidth: 160,
      sortingField: 'StatusDisplayText'
    },
    {
    ...
    {
      headerName: 'Who',
      field: 'CreatedByUserWithEmail',
      filter: 'agTextColumnFilter',
      editable: false,
      minWidth: 200,
      sortingField: 'CreatedByUserWithEmail',
      flex: 0,
      resizable: true,
    },
  ];
      ...
      getRows: (params: any) => {
        ...
        if (paramsRequest.sortModel.length > 0) {
          Object.values(params['columnApi']['columnController']['columnDefs']).forEach((key: any) => {
            if (key) {
              if (key['field'] === paramsRequest.sortModel[0].colId) {
                data['OrderBy'] = key['sortingField'];
        ...
        this.wirelineService
          .getInventoryNotes(data, this.rowData['InventoryId'])
          .pipe(takeUntil(this._unsubscribeInventory))
          .subscribe(
            async (data: any) => {
              this.inventoryCount = data.TotalCount;

              this.isInventoryExist = data.TotalCount > 0 ? true : false;
              data.TotalCount > 0 ? this.isInventoryNoteExist.emit(true) : this.isInventoryNoteExist.emit(false)
              if (data && data.Success && data.Data.$values.length > 0) {
                this.inventoryRowData = data.Data.$values;
                let lastRow = -1;
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['buttonAction']?.currentValue == 'active') {
      this.setInventoryNotes('active')
    } else if (changes['buttonAction']?.currentValue == 'inactive') {
      this.setInventoryNotes('inactive')
    } else if (changes['buttonAction']?.currentValue == 'add') {
      this.addNotes();
    }
  }
  ...
  ngOnDestroy(): void {
    ...
    this._unsubscribeDeactive.next(null);
    this._unsubscribeDeactive.complete();
    this._unsubscribeGRid.next(null);

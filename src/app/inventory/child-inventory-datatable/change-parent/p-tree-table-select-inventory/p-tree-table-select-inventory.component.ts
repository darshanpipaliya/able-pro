...
@Component({
  selector: 'app-p-tree-table-select-inventory',
  templateUrl: './p-tree-table-select-inventory.component.html',
  styleUrls: ['./p-tree-table-select-inventory.component.scss'],
  imports: [SharedModule, PrimgModule],
})
export class PTreeTableSelectInventoryComponent implements OnInit {
  ...
  setCols() {
    ...
    this.cols = [
      ...
      createColumn(4, '150px', true, 'text', 'Product', 'VendorProductName', 'Vendor Product'),
  ...
  loadNodes(event: any, allOptionsClear = false, fromHeaderCheckBox = false) {
    ...
    const data = {
      ...
      ...(this.inventoryData.CustomerAccountId) ? { CustomerAccountId: this.inventoryData.CustomerAccountId } : {},
      ...(this.inventoryData.VendorAccountId) ? { VendorAccountId: this.inventoryData.VendorAccountId } : {},
